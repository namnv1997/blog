    ...
    <div class="row" style="float: left;">
        ...
        <div id="post_display">
            <?php foreach ($info as $row) : ?>
                <div class="row-table">
                    <div class="column-table text-right minw30"><?php echo $row['id'] ?></div>
                    <div class="column-table padding10 minw200"><?php echo $row['title'] ?></div>
                    <div class="column-table padding10 text-summary minw400"><?php echo $row['summary'] ?></div>
                    <?php if ($row['status'] == 1) {
                        echo '<div style="min-width: 80px" class="column-table text-center">Enabled</div>';
                    } else {
                        echo '<div style="min-width: 80px" class="column-table text-center">Disabled</div>';
                    }
                    ?>
                    <div class="column-table text-center minw200" data-id=<?php echo $row['id'] ?>>
                        <button type="button" class="btn btn-primary btnUD edit-post" data-backdrop="static"
                                data-keyboard="false" data-toggle="modal" data-target=".edit-post-modal"> &nbsp Edit
                            &nbsp
                        </button>
                        <button class="btn btn-danger btnUD delete-post" data-toggle="modal"
                                data-target=".delete-post-modal">Delete
                        </button>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        <ul class="pagination post-previous">
            <li class="page-item previous-li-post" style="display: none"><a class="page-link">Previous</a></li>
        </ul>
